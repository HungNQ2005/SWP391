...
public class PerformersAdmin extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            ...
            req.setAttribute("totalPages", Math.max(1, (int) Math.ceil((double) total / PAGE_SIZE)));
            ...
            session.setAttribute("error", "Failed to load performer list. Please try again.");
            resp.sendRedirect(req.getContextPath() + "/admin/performer?page=1");
        }
    }
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        String currentPage = req.getParameter("currentPage") != null ? req.getParameter("currentPage") : "1";
        ...
        try {
            ...
            switch (action) {
                case "add":
                    handleAdd(req);
                    successMsg = "Performer added successfully!";
                    break;
                case "edit":
                    handleEdit(req);
                    successMsg = "Performer updated successfully!";
                    break;
                case "delete":
                    handleDelete(req);
            ...
            log("Validation error in doPost: " + e.getMessage());
            session.setAttribute("error", e.getMessage());
        } catch (Exception e) {
            log("System error in doPost", e);
    ...
    private void handleAdd(HttpServletRequest req) throws Exception {
        ...
        if (dao.existsPerformer(p.getName(), p.getDateOfBirth(), p.getNationality(),
                p.getGender(), p.getPhotoUrl(), p.getDescription())) {
        ...
        if (!dao.addPerformers(p)) {
            throw new IllegalStateException("Failed to add performer to database");
        }
    }
    ...
    private void handleDelete(HttpServletRequest req) throws Exception {
        ...
        int id = Integer.parseInt(idStr);
        if (!dao.deletePerformers(id)) {
            throw new IllegalStateException("Performer not found or already deleted");
        }
    }
    ...
    private List<String> validatePerformer(Performers p) {
        List<String> errors = new ArrayList<>();

        if (p.getName().isEmpty()) {
            errors.add("Name is required");
        } else if (!p.getName().matches("^[\\p{L} .'-]+$")) {
            errors.add("Name must contain only letters and symbols like .'-");
        }

        if (p.getPerformerID() == 0 && (p.getPhotoUrl() == null || p.getPhotoUrl().isEmpty())) {
            errors.add("Photo URL is required");
        }

        if (p.getGender().isEmpty() || !p.getGender().matches("Male|Female")) {
            errors.add("Gender must be Male or Female");
        }

        if (p.getDateOfBirth().isEmpty()) {
            errors.add("Date of birth is required");
        } else {
            try {
                LocalDate date = LocalDate.parse(p.getDateOfBirth());
    ...
    private int getValidPage(String pageParam, String keyword) {
        ...
        try {
            if (pageParam != null && pageParam.matches("\\d+")) {
                ...
                int total = keyword.isEmpty() ? dao.countPerformers() : dao.countPerformersByKeyword(keyword);
    ...
    private String buildRedirectUrl(HttpServletRequest req, String currentPage, String keyword) throws IOException {
        StringBuilder url = new StringBuilder(req.getContextPath())
                .append("/admin/performer?page=").append(currentPage.isEmpty() ? "1" : currentPage);
        if (!keyword.isEmpty()) {
