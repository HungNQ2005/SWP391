...
/**
 *
 * @author Chau Tan Cuong - CE190026, Nguyen Quoc Hung - CE190870, Nguyen Van
 * Phi - CE190465
 */
...
public class AdminCategoryController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (action.equals("delete")) {
            ...
            int count = adminDAO.countSeriesByCategory(id);

            if (count > 0) {
                // Lấy danh sách tên phim
                List<String> filmList = adminDAO.getSeriesNamesByCategory(id);
                List<Category> list = adminDAO.getAllCategories();
                // Gửi sang JSP
                request.setAttribute("filmList", filmList);
                request.setAttribute("categoryName", adminDAO.getCategoryById(id).getName());
                request.setAttribute("deleteWarning", "hasSeries"); 
                request.setAttribute("listCategory", list);
                 // Forward để hiển thị thông báo
                request.getRequestDispatcher("/Admin/CategoryDashboard.jsp").forward(request, response);
                return;
            }
            ...
            response.sendRedirect("category?action=list&success=deleted");
            return;
        }
